...
export default function AddInvoices(props) {
  ...
  // this will show the pdf on the webpage to generate image from webpage
  ...
  const handleSubmit = async (event) => {
    ...
    // Capture the HTML form as an image using html2canvas

    const canvas = await html2canvas(pdfLayOutRef.current);
    const imageData = canvas.toDataURL();
    // setImg(imageData);

    // Set the size and orientation of the PDF document
    // pdf.addImage(imageData, "PNG", x axis gap, y axis gap,width, height );
    pdf.addImage(imageData, "PNG", 0, 0, 200, 300); // Adjust the width and height as needed
    // Save the PDF document
    // Generate base64 data for PDF
    const pdfData = pdf.output("blob"); // Generate the PDF as a Blob object
    ...
    formData.append("pdf", pdfData, "output.pdf");
  ...
  return (
    <div>
        ...
        <div className=" w-full flex  justify-center items-center ">
          <div ref={pdfLayOutRef}>
            <InvoicePdf
              ...
              formData={formData}
              getTotalPaidByClient={getTotalPaidByClient()}
              currentProduct={currentProduct}
              selectedProduct={selectedProduct}
              getTotalProductPriceSum={getTotalProductPriceSum()}
              getTotal={getTotal()}
            />
          </div>
        </div>
                ...
                <div className="border rounded-sm p-2 mb-4">
                  ...
                  <label className="block">
                    ...
                    <select
                      ...
                      onChange={(event) => handleInputChange(event)}
                      className="w-full mt-1 p-1 border-gray-300 border flex items-center justify-center rounded-md"
                    >
                      <option value="percentage" className="py-2">
                        Percentage
                      </option>
      ...
      {isShowPdf && (
        <div className="">
          <div className="md:max-w-[90%] lg:max-w-[40%] gap-3 justify-around mx-auto flex bg-white p-4 rounded-md">
            <button
              type="button"
              onClick={() => setIsShowPdf(false)}
              className=" bg-blue-500 flex-1  text-white px-4 py-2 rounded-md"
            >
              Cancel
            </button>
            <button
